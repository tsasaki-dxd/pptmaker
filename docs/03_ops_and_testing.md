...
**Phase 1 はシンプルに**（単一アカウント・単一ステージ、Lambda 対象）

```
Source: ECR Image (repo=slideforge/api, tag=prod)
  ↓
[deploy-prod]    ── CDK deploy（Prod Stack）
                     + CodeDeploy（Lambda alias traffic shift:
                       Canary10Percent10Minutes → 100%）
  ↓
[post-deploy]    ── メトリクス自動監視（10分）、アラームで自動ロールバック
```

- **main マージ＝本番デプロイ**の 1 本構成で Phase 1 を運用（Stg / 手動承認 / Smoke ステージは持たない）
- 50 名の社内利用規模では Stg / 手動承認のオーバーヘッドがコスト・リードタイム両面で過剰なため
- 品質ゲートは PR 段階の GHA（lint / unit / integration / VRT / pptx-lint）で担保
- Lambda の段階デプロイは `CodeDeployDefault.LambdaCanary10Percent10Minutes`
- CloudWatch Alarm（5xx / エラー率 / レイテンシ）と連動で自動ロールバック

**Phase 2 以降**：Stg 環境を独立 AWS アカウントとして新設し、CodePipeline をクロスアカウント構成に。`deploy-stg → smoke-stg → approval-prod → deploy-prod` の 4 ステージ構成へ拡張。Render 層は ECS Fargate Blue/Green（CodeDeploy `ECSCanary10Percent5Minutes`）に切替。
...
**Phase 1（Lambda alias、単一ステージ）**
- CodeDeploy の Lambda デプロイ構成 `LambdaCanary10Percent10Minutes` を使用
- 新バージョンに 10% → 10 分通流 → 100% 昇格
- CloudWatch Alarm 発報で自動ロールバック（alias が旧バージョンに戻る）
- Stg / Dev 環境は持たないため、Prod のみ Canary で保護する

**Phase 2（ECS Fargate Blue/Green）**
- CodeDeploy `ECSCanary10Percent5Minutes` に移行
- 新タスク 10% に 5 分通流 → メトリクス正常で 100% → 旧タスク 5 分温存 → 破棄
- ロールバック: CloudWatch アラーム連動で 30 秒以内に Blue 切戻し
...
| ツール | 判定 | 理由 |
|---|---|---|
| **AWS CDK (Python)** | **採用** | 開発スタックが Python、`aws-cdk-lib/pipelines` の自己更新パイプライン、Lambda/CodeDeploy ヘルパー充実 |
| Terraform | 採用しない | 社内標準が CDK になる想定 |
| CloudFormation 直書き | 採用しない | CDK の下層、直接は扱わない |
| Console 手動 | 採用しない | 再現性・監査性欠如 |

### 15.2 Phase 1 ディレクトリ構成

```
pptmaker/
├── app/                         # アプリ本体（Lambda ハンドラ含む）
├── infra/                       # CDK アプリ
│   ├── app.py                   # エントリポイント
│   ├── cdk.json
│   ├── requirements.txt
│   ├── stacks/
│   │   ├── pipeline_stack.py    # CodePipeline + CodeBuild
│   │   ├── data_stack.py        # RDS t4g.small, S3, KMS, Secrets
│   │   ├── app_stack.py         # API Gateway + Lambda + SQS + Render Lambda
│   │   └── obs_stack.py         # CloudWatch Alarms, Dashboards
│   ├── stages/
│   │   └── app_stage.py         # Prod 単一 Stage（Phase 2 で Stg / Dev を追加）
│   └── tests/
└── .github/workflows/
```

### 15.3 Phase 1 の対象リソース（単一アカウント）

CDK が管理するもの：

- API Gateway（REST or HTTP API）
- Lambda Function（API）、Lambda Function（Render / Container Image）
- SQS Queue（Render 非同期実行用）
- RDS db.t4g.small Single-AZ + 専用 VPC（RDS 接続用の最小 Subnet）
- S3 バケット（templates / projects / previews / logs）
- KMS CMK、Secrets Manager エントリ
- Cognito User Pool、App Client
- CloudWatch Log Group、Alarm、Dashboard
- IAM Role（Lambda 実行、CodeDeploy、EventBridge）
- CodePipeline、CodeBuild、CodeDeploy（Lambda alias 用）
- ECR リポジトリ（Render Lambda Container）

**Phase 1 で IaC 対象外**（存在自体しない）：
- NAT Gateway、CloudFront Distribution、WAF WebACL、ElastiCache、ALB、ECS Cluster、GuardDuty、Aurora、CRR

手動管理：
- AWS アカウント、Route53 親ゾーン、Identity Center ユーザー/グループ
...
[SelfMutate] cdk synth → 自身の CloudFormation 差分適用
  ↓
[Deploy-Prod Stack]（Lambda alias Canary 10% → 100%）
```

パイプライン定義の変更も同じ PR フローで入る。**手動 `cdk deploy` は初回のみ**。

### 15.5 環境分離（Phase 1：単一アカウント・単一ステージ）

```
Single AWS Account
├── SlideForgePipelineStack     （Pipeline 本体）
└── SlideForge-Prod-*            （Prod Stack 群）
```

- Phase 1 は **Prod のみ**（main マージで即本番デプロイ）
- 50 名の社内利用に Stg / 手動承認は過剰なため、品質ゲートは PR 段階の GHA に集約
- IAM / S3 / Secrets / RDS はリソース名に `env=prod` を付与（将来の環境追加に備える）
- Phase 2 でマルチアカウント化する際は `Stage` を追加するだけで移行可能に設計

### 15.6 初回ブートストラップ

詳細手順は **`docs/bootstrap.md`** を参照。Phase 1 は以下の簡略版で 1〜2 時間で完了する：

1. 単一 AWS アカウントで `cdk bootstrap`
2. IAM OIDC Provider + GHA 用 Role を作成
3. GitHub リポジトリに Variables 登録
4. ECR リポジトリ作成
5. 管理者が 1 回だけ `cdk deploy SlideForgePipelineStack`
6. 以降は PR フローのみ

### 15.7 IaC の CI

- `infra/` に変更が入った PR で `cdk synth` / `cdk diff` を GHA で実行
- `cdk-nag` で AWS Solutions 規格の静的チェック
- Plan 差分を PR コメントに貼る

### 15.8 Phase 2 移行の計画

Phase 2 開始時点での主な再設計タスク：

| 項目 | 移行作業 |
|---|---|
| AWS アカウント分離 | Dev/Stg/Prod を独立アカウント化、各アカウントで `cdk bootstrap --trust`、Pipeline を CrossAccount 構成に変更 |
| DB 切替 | RDS t4g.small → Aurora Serverless v2、Blue/Green または論理レプリケーションで移行 |
| API 実行基盤 | Lambda → ECS Fargate + ALB、段階移行（一部エンドポイントから順次） |
| 追加コンポーネント | CloudFront、WAF、ElastiCache、GuardDuty の CDK 導入 |
| 想定工数 | 1.5 人月（Phase 2 の 4 週間に割当） |
