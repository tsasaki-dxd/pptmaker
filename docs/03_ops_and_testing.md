...
**Phase 1 はシンプルに**（単一アカウント、Lambda 対象）

```
Source: ECR Image (repo=slideforge/api, tag=prod)
  ↓
[deploy-stg]     ── CDK deploy（Stg Stack、Lambda エイリアス更新）
  ↓
[smoke-stg]      ── CodeBuild（E2E 抜粋、ヘルスチェック）
  ↓
[approval-prod]  ── 手動承認（Slack 通知 + AWS Console 承認）
  ↓
...
- **単一 AWS アカウント内の Stack 分離**（Dev/Stg/Prod）で Phase 1 を運用
- Lambda の段階デプロイは `CodeDeployDefault.LambdaCanary10Percent10Minutes`
- CloudWatch Alarm（5xx / エラー率 / レイテンシ）と連動で自動ロールバック

**Phase 2 以降**：マルチアカウントに移行し、Dev/Stg/Prod 間は CodePipeline のクロスアカウント実行、Render 層は ECS Fargate Blue/Green（CodeDeploy `ECSCanary10Percent5Minutes`）に切替。
...
- Dev / Stg は `LambdaAllAtOnce`（即時 100%）
...
│   ├── stages/
│   │   └── app_stage.py         # Dev/Stg/Prod 共通 Stage（同一アカウント内）
│   └── tests/
└── .github/workflows/
```
...
[SelfMutate] cdk synth → 自身の CloudFormation 差分適用
  ↓
[Deploy-Stg Stack] → [Smoke Test] → [Approval] → [Deploy-Prod Stack]
```

パイプライン定義の変更も同じ PR フローで入る。**手動 `cdk deploy` は初回のみ**。

### 15.5 環境分離（Phase 1：単一アカウント）

```
Single AWS Account
├── SlideForgePipelineStack     （Pipeline 本体）
├── SlideForge-Dev-*            （Dev Stack 群）
├── SlideForge-Stg-*            （Stg Stack 群）
└── SlideForge-Prod-*           （Prod Stack 群）
```

- Stack 名 prefix で環境を分離
- IAM / S3 / Secrets / RDS はリソース名で `env=dev|stg|prod` を付与
- Dev / Stg は idle 時のコストが事実上 $0（Lambda 従量、RDS 停止スクリプトで夜間停止）
- Phase 2 でマルチアカウント化する際は `Stage` 単位で `env` 指定を切替えるだけで移行可能に設計
